...
namespace OpenTK_winforms_z02 {
    public partial class Form1 : Form {
        ...
        private Timer movementTimer = new Timer(); // Timer pentru deplasarea obiectelor
        private Vector3 cameraPosition = new Vector3(0, 0, 10);
        private Vector2 lastMousePosition;
        private bool isDraggingObject = false; // Drag-and-drop pentru obiecte
        ...
        {
            // Inițializăm parametrul timerului
            movementTimer.Interval = 50; // 50ms
            ...
            rotationTimer.Tick += OnRotationTick;

            // Evenimente pentru GLControl
            GlControl1.Paint += GlControl_Paint;
            GlControl1.MouseClick += GlControl_MouseClick;
            GlControl1.KeyDown += GlControl_KeyDown;
        ...
        private void DeseneazaCubQ_Tex1() {
            ...
            switch (colorTex) {
                case 0:
                    GL.Color3(Color.White);      //Culoarea albă prmite maparea texturii fără alterarea culorii originale.
                    break;
                case 1:
        ...
        private void LoadObjectFromFile(string filePath)
        {
            try
            {
                ...
                float scaleFactor = 30.0f; // Obiectul va fi de 10 ori mai mare
                ...
                foreach (var line in lines)
                {
                    ...
                    if (parts.Length == 3)
                    {
                        obj.Vertices.Add(new Vector3(
                            float.Parse(parts[0]) * scaleFactor,
                            float.Parse(parts[1]) * scaleFactor,
                            float.Parse(parts[2]) * scaleFactor
                        ));
                    }
                }

                obj.GenerateFaces(); // Generăm fețele cubului
                //obj.Color = GenerateRandomColor();
                objects.Add(obj);

                Log($"Loaded object with {obj.Vertices.Count} vertices and {obj.Faces.Count} faces, scaled by {scaleFactor}.");
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error loading object: {ex.Message}");
            }
        }
        ...
        private void GlControl_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            {
                selectedObject = GetObjectUnderMouse(e.X, e.Y);
                if (selectedObject != null)
                {
                    isDraggingObject = true;
                    lastMousePosition = new Vector2(e.X, e.Y);
                    Log($"Selected object at position {selectedObject.Position}");
                }
            }
        }
        ...
        private void GlControl_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            {
                Vector2 delta = new Vector2(e.X - lastMousePosition.X, e.Y - lastMousePosition.Y);
                selectedObject.Translate(delta.X * 0.01f, -delta.Y * 0.01f, 0);
                lastMousePosition = new Vector2(e.X, e.Y);
                GlControl1.Invalidate();
            }
            else if (isCameraControlled && e.Button == MouseButtons.Left)
            {
                Vector2 delta = new Vector2(e.X - lastMousePosition.X, e.Y - lastMousePosition.Y);
                cameraPosition += new Vector3(delta.X * 0.01f, -delta.Y * 0.01f, 0);
                lastMousePosition = new Vector2(e.X, e.Y);
                GlControl1.Invalidate();
            }
        }
        ...
        private void btnRotate_Click(object sender, EventArgs e)
        {
            ...
            if (selectedObject != null)
            {
                if (isRotating == true)
                {
                    rotationTimer.Start(); // Rotește în jurul axei Y
                    Log($"Rotated object: {selectedObject.Rotation}");
                    GlControl1.Invalidate();
                }
                else
                {
        ...
        private void OnRotationTick(object sender, EventArgs e)
        {
            if (selectedObject != null)
            {
                selectedObject.Rotate(1, 1, 0.5f, 0.2f); // Rotește pe toate axele
                GlControl1.Invalidate();
            }
        }
        ...
        private void btnScale_Click(object sender, EventArgs e)
        {
            if (selectedObject != null)
            {
                selectedObject.Scale(1.1f, 1.1f, 1.1f); // Scalează uniform
                Log($"Scaled object to {selectedObject.ScaleFactors}");
                GlControl1.Invalidate();
            }
        }
        ...
        private void btnTranslate_Click(object sender, EventArgs e)
        {
            if (selectedObject != null)
            {
                selectedObject.Translate(0.5f, 0, 0); // Translatează pe axa X
                Log($"Translated object to {selectedObject.Position}");
                GlControl1.Invalidate();
            }
        }
        ...
        private Object3D GetObjectUnderMouse(int x, int y)
        {
            // Implementăm selecția bazată pe coordonate 2D și proiectarea obiectelor
            foreach (var obj in objects)
            {
                if (obj.IsMouseOver(x, y, GlControl1.Width, GlControl1.Height, new Vector3(eyePosX, eyePosY, eyePosZ)))
                {
                    return obj;
                }
            }
            return objects.Count > 0 ? objects[0] : null;
            // Placeholder: selectăm primul obiect pentru demonstrație
        }

        // Control obiecte cu taste
        private void GlControl_KeyDown(object sender, KeyEventArgs e)
        {
            if (selectedObject != null)
            {
                switch (e.KeyCode)
                {
                    case Keys.W: selectedObject.Translate(0, 0.1f, 0); break;
                    case Keys.S: selectedObject.Translate(0, -0.1f, 0); break;
                    case Keys.A: selectedObject.Translate(-0.1f, 0, 0); break;
                    case Keys.D: selectedObject.Translate(0.1f, 0, 0); break;
                    case Keys.Q: selectedObject.Translate(0, 0, 0.1f); break;
                    case Keys.E: selectedObject.Translate(0, 0, -0.1f); break;
                }
                Log($"Moved object to {selectedObject.Position}");
                GlControl1.Invalidate();
            }
        }

        // Timer pentru deplasare automată
        private void OnMovementTimerTick(object sender, EventArgs e)
        {
            if (selectedObject != null)
            {
                selectedObject.Translate(0.1f, 0, 0);
                Log($"Timer moved object to {selectedObject.Position}");
                GlControl1.Invalidate();
            }
        }
        ...
        private void btnReset_Click(object sender, EventArgs e)
        {
            ...
            Log("Reset scene and objects.");
            GlControl1.Invalidate();
        }

        // Generare culoare aleatoare
